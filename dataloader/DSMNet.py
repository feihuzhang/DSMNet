...
class Feature(nn.Module):
    def __init__(self):
        ...
        self.conv_start = nn.Sequential(
            ...
            BasicConv(32, 32, kernel_size=5, stride=3, padding=1),
...
#        self.conv32x1 = BasicConv(32, 1, kernel_size=3)
    ...
    def forward(self, x):
        x = F.interpolate(self.conv3d_2d(x), [self.maxdisp+1, x.size()[3]*3, x.size()[4]*3], mode='trilinear', align_corners=False)
        x = torch.squeeze(x, 1)
        x = self.softmax(x)
