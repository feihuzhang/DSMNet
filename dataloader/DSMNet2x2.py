...
from libs.GANet.modules.GANet import NLFIter, NLFMean, NLFMax, NLF
...
class DomainNorm(nn.Module):
    def __init__(self, channel, l2=True):
        ...
        self.bn = nn.InstanceNorm2d(num_features=channel, affine=True)
        self.l2 = l2
    def forward(self, x):
        if self.l2:
...
class NLFilter2(nn.Module):

    def __init__(self, in_channel=32):
        super(NLFilter2, self).__init__()
        self.inner_channel = in_channel
        self.conv_refine = BasicConv(in_channel + self.inner_channel, in_channel, bn=True, relu=True, kernel_size=3, stride=1, padding=1)
        self.nlf = NLFIter()
...
class NLFilter(nn.Module):
    ...
    def __init__(self, in_channel=32):
        ...
        self.inner_channel = in_channel
        self.conv_refine = BasicConv(in_channel + self.inner_channel, in_channel, bn=True, relu=True, kernel_size=3, stride=1, padding=1)
        self.getweights = nn.Sequential(BasicConv(in_channel, self.inner_channel, bn=True, l2=False, relu=False, kernel_size=3, stride=1, padding=1),
                                        GetWeights())
...
class Attention3d(nn.Module):
    def __init__(self, in_channel=32):
        ...
        self.conv_refine = BasicConv(in_channel + self.inner_channel, in_channel, is_3d=True, bn=True, relu=True, kernel_size=3, stride=1, padding=1)
...
#        self.conv11 = Conv2x(32, 48)
        ...
        self.getweights1 = nn.Conv2d(32, 20, kernel_size=3, stride=1, padding=1, bias=False)

        self.getweights2 = nn.Conv2d(48, 20, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.weight_lg1 = nn.Sequential(BasicConv(32, 32, kernel_size=3, padding=1),
                                        BasicConv(32, 32, relu=False, l2=False, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        sg1 = self.getweights1(sg1)
        x = self.conv2(x) + rem
        rem = x
        sg2 = self.weight_sg2(x)
        sg2 = self.getweights1(sg2)
        x = self.conv3(x) + rem
        sg3 = self.weight_sg3(x)
        sg3 = self.getweights1(sg3)
        
        x = self.conv11(x)
        rem = x
        sg11 = self.weight_sg11(x)
        sg11 = self.getweights2(sg11)
        x = self.conv12(x) + rem
        rem = x
        sg12 = self.weight_sg12(x)
        sg12 = self.getweights2(sg12)
        x = self.conv13(x) + rem
        rem = x
        sg13 = self.weight_sg13(x)
        sg13 = self.getweights2(sg13)
        x = self.conv14(x) + rem
        sg14 = self.weight_sg14(x)
        sg14 = self.getweights2(sg14)
...
#        self.conv32x1 = BasicConv(32, 1, kernel_size=3)
    ...
    def forward(self, x):
        x = F.interpolate(self.conv3d_2d(x), [self.maxdisp+1, x.size()[3]*4, x.size()[4]*4], mode='trilinear', align_corners=False)
        x = torch.squeeze(x, 1)
        x = self.softmax(x)
...
#        self.conv32x1 = BasicConv(32, 1, kernel_size=3)
    ...
    def forward(self, x, lg1):
        x = F.interpolate(self.conv32x1(x), [self.maxdisp+1, x.size()[3]*4, x.size()[4]*4], mode='trilinear', align_corners=False)
        x = torch.squeeze(x, 1)
        x = self.lgf(x, lg1)
        x = self.softmax(x)
        x = self.lgf(x, lg1)
